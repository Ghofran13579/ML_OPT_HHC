...
def run_ncro_one_file(csv_path: Path, pat_count: int) -> pd.DataFrame:
    ...
    QL_ALPHA, QL_GAMMA, QL_EPSILON = 0.8, 0.7, 0.4
 ...
 # === Q-LEARNING MAIN LOOP (one operator per iteration + Q-table tracking) ===    
    while it < MAX_ITERS and FE < FELIMIT:
        ...
        # AFTER sorting by Rank & CD
        final = []
        for cand in pop_extended:
            if not final:
                final.append(cand)
            else:
                # skip solution if too close to existing ones
                if all(np.linalg.norm(np.array(cand.obj) - np.array(sol.obj)) > 1e-3 for sol in final):
                    final.append(cand)
            if len(final) == POP_SIZE:
                break
        
        pop = final
        combined = global_archive + pop
        assign_rank_cd_pe(combined)
        global_archive = [m for m in combined if m.Rank == 1]
    ...
    try:
        pareto_front = np.array([[m.obj[0], m.obj[1], m.obj[2]] for m in global_archive])
