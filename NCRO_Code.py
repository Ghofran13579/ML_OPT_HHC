...
OUTPUT_FOLDER = r"C:\Users\Ghofran MASSAOUDI\Desktop\ncro_results"
...
def enforce_and_score_one_nurse_CORRECTED(df, nurse_id, patient_sequence, csv_name,
    ...
    for patient_id in patient_sequence:
        ...
        Z3_n += abs(qv - sv)  # Use absolute difference
...
def run_ncro_one_file(csv_path: Path, pat_count: int) -> pd.DataFrame:
    ...
    while it < MAX_ITERS and FE < FELIMIT:
        ...
        # ===== Stage 1: create exactly POP_SIZE offspring =====
        Q: List[Molecule] = []
        for _ in range(POP_SIZE):
            if random.random() < MOLE_COLL and len(pop) >= 2:
                # --- Intermolecular (2 parents) ---
                p1, p2 = random.sample(pop, 2)
                if (p1.KE <= BETA) and (p2.KE <= BETA):
            ...
            # Stop if we reached the required population size
            if len(Q) >= POP_SIZE:
                Q = Q[:POP_SIZE]
                break
        ...
        # ===== Stage 2: compute Rank, CD, PE on P ∪ Q =====
        pool = pop + Q
        assign_rank_cd_pe(pool)

        # ===== Stage 3: energy update per child, using PE scalars =====
        SurvivorsQ: List[Molecule] = []
        removed_parents = set()
        ...
        # ===== Stage 4: build next pool, re-rank, select N best =====
        new_parents: List[Molecule] = []
        for k, m in enumerate(pop):
            if k not in removed_parents:
                new_parents.append(m)
        new_pool = new_parents + SurvivorsQ

        assign_rank_cd_pe(new_pool)

        order = sorted(range(len(new_pool)), key=lambda i: (new_pool[i].Rank, -new_pool[i].CD))
        pop = [new_pool[i] for i in order[:POP_SIZE]]
    ...
    # ===== Hypervolume Calculation (Mme Abir adaptation, normalized) =====
    try:
        ...
        if len(pareto_front) <= 1:
            ...
            # --- Reference front = all solutions ---
            ref_front = df_final[["F1", "F2", "F3"]].to_numpy()

            # --- Convert F3 → -F3 (because F3 is maximized, others minimized) ---
            ...
            max_vals = np.max(ref_min, axis=0)
            min_vals = np.min(ref_min, axis=0)
            norm_pf  = (pareto_min - min_vals) / (max_vals - min_vals + 1e-12)
            norm_ref = (ref_min - min_vals) / (max_vals - min_vals + 1e-12)

            # Clip to [0,1] for stability
            norm_pf  = np.clip(norm_pf, 0.0, 1.0)
            norm_ref = np.clip(norm_ref, 0.0, 1.0)

            # --- Compute HV using Mme Abir’s algorithm ---
            hv_calc = Hypervolume()
            HV_value = hv_calc.hypervolume(norm_pf, norm_ref, 3)
    ...
    # Store HV (same for all rows in this instance)
...
# MULTI-RUN WRAPPER (10 independent runs; reuse inner HV)
# =========================
if __name__ == "__main__":
    RUNS = 10
    all_files = [f for f in Path(INPUT_FOLDER).glob("*.csv")]
    if not all_files:
    ...
    for run_id in range(1, RUNS + 1):
        print(f"\n===============================")
        print(f"🚀 Starting NCRO Run {run_id} / {RUNS}")
        print(f"===============================\n")
        ...
        # Distinct seeds per run (controlled variability)
        ...
        for f in all_files:
            try:
                ...
                # Reuse inner HV (no recomputation)
                hv_value = float(df_out["HV"].iloc[0]) if ("HV" in df_out.columns and len(df_out) > 0) else float("nan")

                # Save per-run result
                out_path = run_folder / f"{f.stem}_Run{run_id:02d}_ncro_results.csv"
                df_out = df_out[["Solution", "F1", "F2", "F3", "Rank", "PE", "HV"]]
                df_out.to_csv(out_path, sep=",", float_format="%.6f", index=False)

                print(f"✅ Run {run_id}: saved {out_path}")
                print(f"   └─ HV = {hv_value:.6f}\n")

            except Exception as e:
                print(f"❌ Run {run_id}: Skipped {f.name} due to error: {e}\n")
