...
def run_ncro_one_file(csv_path: Path, pat_count: int) -> pd.DataFrame:
    ...
    def compute_reward(parents, children):
            """
            Dominance reward (scalar in [0,1]):
            - For each child, reward 1 if it dominates at least one of its parents,
              reward 0 if it is dominated or non-dominated by all available parents.
            Then average over all children considered.
            NOTE: F3 is MAX → compare on (F1, F2, -F3) in minimization form.
            """
            def dom_vec(m):
                # minimization vector (F1, F2, -F3)
                return (m.obj[0], m.obj[1], -m.obj[2])
        
            def dominates(a, b):
                av, bv = dom_vec(a), dom_vec(b)
                better_in_any = False
                for ai, bi in zip(av, bv):
                    if ai > bi:     # worse in any objective → cannot dominate
                        return False
                    if ai < bi:
                        better_in_any = True
            ...
            # Build a quick index-safe list of parents
            P = list(parents)
            ...
            for child in children:
                # Try to use recorded parent indices when available
                pA, pB = getattr(child, "parents", (None, None))
                cand_parents = []
                if pA is not None and 0 <= pA < len(P) and P[pA] is not None:
                    cand_parents.append(P[pA])
                if pB is not None and 0 <= pB < len(P) and P[pB] is not None:
                    cand_parents.append(P[pB])
        
                # Fallback: if we couldn't resolve parents (should be rare), use all
                if not cand_parents:
                    cand_parents = P
        
                # Child earns 1 if it dominates at least one of its parents
                child_wins = any(dominates(child, par) for par in cand_parents)
                total += 1.0 if child_wins else 0.0
                count += 1
