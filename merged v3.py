...
OUTPUT_FOLDER = r"C:\Users\massa\ISG\Q-Learnung_NCRO_Merged-test"
...
def run_ncro_one_file(csv_path: Path, pat_count: int) -> pd.DataFrame:
    ...
    QL_ALPHA, QL_GAMMA, QL_EPSILON = 0.1, 0.9, 0.4
    prev_action = 0  # start assuming 'wall' was last operator
    def compute_reward(parents, children):
        """
        Enhanced reward:
        +1 if child improves 1 objective,
        +2 if improves 2 objectives,
        +3 if improves all 3 objectives,
        -1 if improves none.
        ...
        def improvement(child_obj, parent_obj):
            """Count how many objectives improved relative to a parent."""
            c1, c2, c3 = child_obj
            p1, p2, p3 = parent_obj
            improved = 0
            if c1 < p1:  # F1 improved (minimize)
                improved += 1
            if c2 < p2:  # F2 improved (minimize)
                improved += 1
            if c3 > p3:  # F3 improved (maximize)
                improved += 1
            return improved
        ...
        for child in children:
            ...
            cand_parents = []
            if pA is not None and 0 <= pA < len(parents) and parents[pA] is not None:
                cand_parents.append(parents[pA])
            if pB is not None and 0 <= pB < len(parents) and parents[pB] is not None:
                cand_parents.append(parents[pB])
            if not cand_parents:
                cand_parents = parents  # fallback
    
            best_improv = 0
            for par in cand_parents:
                if par is None:
                    continue
                imp = improvement(child.obj, par.obj)
                best_improv = max(best_improv, imp)
    
            # Assign reward per child
            if best_improv == 0:
                total_reward += -1
            else:
                total_reward += best_improv
            count += 1
...
def assign_rank_cd_pe(pool: List[Molecule]) -> None:
    ...
    for i, m in enumerate(pool):
        m.CD = float(CD[i])
        m.PE = float(m.Rank + 0.7 * math.exp(-max(0.0, m.CD)))
# =========================
