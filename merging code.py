...
OUTPUT_FOLDER = r"C:\Users\Ghofran MASSAOUDI\Desktop\Q-Learnung_NCRO_results_merged"
...
def run_ncro_one_file(csv_path: Path, pat_count: int) -> pd.DataFrame:
    ...
    def compute_reward(parents, children):
        """Positive reward when offspring improve objective quality."""
        def score(m):  # smaller = better
            return m.obj[0] + m.obj[1] - m.obj[2]
        p_avg = np.mean([score(p) for p in parents if p is not None])
        c_avg = np.mean([score(c) for c in children])
        return max(0.0, p_avg - c_avg)
    
  # === Q-LEARNING MAIN LOOP ===
    while it < MAX_ITERS and FE < FELIMIT:
        it += 1
    
        # 1️⃣ --- Compute baseline mean fitness for Q-reward ---
        base_fit = np.mean([fitness(m) for m in pop])
    
        # 2️⃣ --- Select operator using ε-greedy policy ---
        state = prev_action
        if random.random() < QL_EPSILON:
            action_idx = random.randint(0, len(ACTIONS) - 1)
        else:
            action_idx = int(np.argmax(Q_table[state]))
        action = ACTIONS[action_idx]  # one of ["wall","dec","inter","syn"]
        ...
        reward  = float(np.clip(base_fit - new_fit, -1.0, 1.0))
...
# =========================
# FAULT-TOLERANT MULTI-RUN WRAPPER (PER-RUN FOLDERS)
# =========================
NUM_RUNS = 5   # ← Change to 10 if you want 10 runs
